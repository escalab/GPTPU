        ...
        float dt, int numiter) 
{
    ...
    cudaFree(tIn_d);
    cudaFree(tOut_d);
    double us = std::chrono::duration_cast<std::chrono::nanoseconds>(Myend - Mystart).count()/1000.0;
    printf("GPU total time (no setting): %12.3f (us) \n", us);
    return;
}
